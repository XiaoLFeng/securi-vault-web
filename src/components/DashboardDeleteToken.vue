...
import type {Token} from "@/models/dto/tokenDTO";
import {delTokenApi} from "@/apis/token-api";
...
export default defineComponent({
  ...
  methods: {
    async handleOk() {
      if (this.delTokenUuid === "") {
        message.warn('删除信息不能为空');
      }
      this.confirmLoading = true;
      const returnData = await delTokenApi(this.delTokenUuid.id, this.authorizationCode);
...
<template>
  <a-modal v-model:open="modal" title="删除令牌">
    <div>
      <span>确定要删除 <span class="font-bold">{{ delTokenUuid.site }}</span> 中 <span
          class="font-bold">{{ delTokenUuid.id }}</span> 的密码吗？这将会失去很长很长时间......</span>
    </div>
